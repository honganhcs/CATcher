...
const UI_FLAW_DEFINITION = '<p>A flaw in the user interface' + '<span style="color:grey;">e.g., typos, components overlap, etc.</span></p>';
...
export class LabelService {
  ...
  private ensureRepoHasRequiredLabels(actualLabels: Label[], requiredLabels: Label[]): void {
    ...
    actualLabels.forEach((label) => {
      const nameMatchedLabels: Label[] = requiredLabels.filter((localLabel) => localLabel.getFormattedName === label.getFormattedName);
